...
{
  template <typename Type> Type getBinIndex(float const& value, std::vector<float> const& binning, int const& NsubBins = 1){
    Type res = -100;
...
template <typename TrackType> float FemtoPair<TrackType>::GetKt() const {
  if(_first == NULL || _second == NULL) return -1000;
  if(!(_magfield1*_magfield2)) return -1000;
  if(!(_PDG1*_PDG2)) return -1000;
